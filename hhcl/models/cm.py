...
class ClusterMemory(nn.Module, ABC):
    ...
    def __init__(self, num_features, run, num_samples, temp=0.05, momentum=0.2, mode='CM', hard_weight=0.5, smooth=0., num_instances=1):
    ...
    def forward(self, inputs, targets):
        if self.cm_type in ['CM', 'CMhard']:
            ...
            out = torch.stack(out_list[1:], dim=0)  #(8,64,115)
            neg = torch.max(out, dim=0)[0]  #(64,115)
            pos = torch.min(out, dim=0)[0]  #(64,115)
            mask = torch.zeros_like(out_list[0]).scatter_(1, targets.unsqueeze(1), 1)   #(64,115)
            logits = mask * pos + (1-mask) * neg    #(64,115)
