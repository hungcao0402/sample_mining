...
class CM_Hybrid_Least(autograd.Function):

    @staticmethod
    def forward(ctx, inputs, targets, features, momentum, epoch):
        ctx.features = features
        ctx.momentum = momentum
        ctx.save_for_backward(inputs, targets, epoch)
        outputs = inputs.mm(ctx.features.t())
...
def cm_hybrid_least(inputs, indexes, features,  momentum=0.5):
    return CM_Hybrid_Least.apply(inputs, indexes, features, torch.Tensor([momentum]).to(inputs.device))
...
class ClusterMemory(nn.Module, ABC):
    ...
    def __init__(self, negative_sample, positive_sample, num_features, run, num_samples, temp=0.05, momentum=0.2, mode='CM', hard_weight=0.5, smooth=0., num_instances=1):
    ...
    def forward(self, inputs, targets, epoch):
        if self.cm_type in ['CM', 'CMhard']:
            ...
            loss_output_hard = self.cross_entropy(output_hard, targets)
            loss_centroid = self.cross_entropy(output_mean, targets)
